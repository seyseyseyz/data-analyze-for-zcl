...
_LEVER_PARETO = "GMV 高度集中在少数头部 SKU → 这周先保住头部 SKU 的库存和加投、别让它断货，再从腰部挑潜力款小额测新，慢慢养出第二梯队。"
_LEVER_REFUND = "高退款 SKU → 这周先从退款最高的几个 SKU 入手，逐条核对详情页/尺寸描述是否与实物一致、发货时效有没有拖后腿，再针对性优化退货流程。"
_LEVER_CONVERSION = "加购转化偏低或客单价失衡 → 这周先给加购多但成交少的 SKU 补一个更直接的详情页转化钩子；高客单 SKU 加赠权益打消犹豫，低客单 SKU 试试搭配销售拉高客单。"
...
_LEVER_PRICE_BAND = (
    "GMV 若集中在某价位带 → 这周就把主推和库存资源集中到该带、别断货；再往空档价位带测新品补齐，"
    "别让流量和货盘只押在单一价位上。"
)
...
_LEVER_SWEET_SPOT = (
    "甜点价位带（转化高且退款低）→ 这周优先给该带补货盘、加主推，把好卖又少退的钱先赚到；"
    "高转化但高退款的价位带先治退款再放量，低转化价位带优化承接页或阶段性收缩。"
)
...
def _pareto_finding(
    ...
    conclusion = (
        ...
        f"要凑够 80% 的 GMV，得靠 {qty(skus_for_80pct)} 个 SKU。"
...
def _refund_finding(
    ...
    conclusion = (
        ...
        f"其中 {qty(fdr_survivors)} 个排掉碰巧偏高的之后，还是实打实地高。"
    )
...
def _price_band_distribution_finding(
    ...
    finding = Finding(
        ...
        caveats=[
            ...
            "价位带的分法跟退款根因诊断那节一样（都按客单价高低分档），方便两边对着看。",
        ],
...
def _price_sweet_spot_finding(
    ...
    if sweet is not None:
        ...
        conclusion = (
            f"整体加购转化 {round(overall_conv * 100, 1)}%、退款率 {round(overall_refund * 100, 1)}%。"
            "没有哪个价位带能做到转化不比整体差、退款也不比整体高，暂时找不出明确的甜点带。"
        )
    ...
    finding = Finding(
        ...
        caveats=[
            ...
            "价位带的分法跟价格带分布、退款根因那几节一样（都按客单价高低分档），方便几边对着看。",
            "转化和退款是把一个价位带里的 SKU 合起来算的，不是一个个顾客算的；甜点带只是数字上看着最划算，不代表就是它带来的。",
        ],
...
def _conversion_caveats(conversion_universe: int, gmv_universe: int | None) -> list[str]:
    ...
    if gmv_universe is not None:
        caveats.append(
            f"本节统计的是「加购人数>0」的 SKU（{qty(conversion_universe)} 个），"
            f"与 GMV 集中度的「GMV>0」有效 SKU（{qty(gmv_universe)} 个）不同："
            f"有加购但未成交、无 GMV 的 SKU 只计入本节，不计入 GMV 集中度，所以两处 SKU 数不一致。"
        )
    return caveats
...
# --------------------------------------------------------------------------- #
_LEVER_L2 = (
    "高营收的二级品类，这周先稳住选品与流量、别断供；高退款的二级品类单独拉出来复核尺寸/描述/发货哪一环出问题，"
    "把营收贡献与退款风险分开管理，别一刀切。"
)
...
def _category_l2_finding(
    ...
    conclusion = (
        f"共 {qty(len(l2_rows))} 个二级品类，最赚钱的是 {top_gmv_category}"
    ...
    if top_refund is not None and top_refund["category_l2"] != top_gmv_category:
        conclusion += (
            ...
            f"跟最赚钱的那个不是同一个品类，得分开管。"
        )
    elif top_refund is not None:
        conclusion += (
            f" 最赚钱的 {top_gmv_category} 同时也是退款率最高的品类"
    ...
    if not has_refund:
        ...
        caveats.append(
            f"退款率只算成交单数 ≥{_MIN_ORDERS_GUARD} 的二级品类，单子太少的品类退款率就留空，免得数字不准。"
        )
