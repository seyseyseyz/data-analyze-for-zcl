...
_LEVER_TREND_DECLINE = "先止跌：这周逐个排查搜索承接页和词-货匹配，看问题是出在承接页还是词没对上货。"
_LEVER_OPPORTUNITY = "这周先给高机会词加投，并为它们补上定向笔记和商详承接。"
_LEVER_LEAK = "这周先给高流失词降权减投，再逐一修词-货匹配和承接页相关性。"
_LEVER_CLICK_LEAK = "这些词高曝光却少人点（卡在点击漏损）：这周先换封面/标题、对齐词-货匹配，把点击拉起来。"
_LEVER_CONV_LEAK = "这些词有人点却少下单（卡在转化漏损）：这周先优化商详、价格和信任状承接，把转化补上。"
...
def _carrier_finding(con, limitations: list[str]) -> tuple[Finding, list[dict]]:
    ...
    caveats = [_OBS_CAVEAT]
    if not has_buyers:
        caveats.append("成交人数为估算值：无 paid_buyers，由 曝光×点击率×成交转化率 正推。")
